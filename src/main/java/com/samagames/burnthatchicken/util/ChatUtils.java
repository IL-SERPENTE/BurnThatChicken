...
public class ChatUtils
{
	...
	public static void sendBigMessage(Player p, String msg, int fadein, int duration, int fadeout)
	{
		...
		((CraftPlayer)p).getHandle().playerConnection.sendPacket(new PacketPlayOutTitle(EnumTitleAction.TITLE, title));
	...
	{
		IChatBaseComponent title = ChatSerializer.a("{'text': \"" + msg + "\"}");
		((CraftPlayer)p).getHandle().playerConnection.sendPacket(new PacketPlayOutTitle(EnumTitleAction.SUBTITLE, title));
		((CraftPlayer)p).getHandle().playerConnection.sendPacket(new PacketPlayOutTitle(fadein, duration, fadeout));
	}
