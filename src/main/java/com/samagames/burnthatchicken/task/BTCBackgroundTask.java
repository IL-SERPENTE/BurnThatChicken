...
public class BTCBackgroundTask implements Runnable {
	...
	public void spawnChicken(BTCGameZone zone, boolean canSpecial) {
		if (zone.isEnded())
			return;
		Location spawn = this.randomLocationInZone(
				zone.getChickenSpawnZone()[0], zone.getChickenSpawnZone()[1]);
		Chicken e = (Chicken) spawn.getWorld().spawnEntity(spawn,
				EntityType.CHICKEN);
		if (canSpecial && random.nextInt(15) == 0) {
			Location spawn2 = this.randomLocationInZone(
					zone.getChickenSpawnZone()[0],
					zone.getChickenSpawnZone()[1]);
			Chicken e2 = (Chicken) spawn2.getWorld().spawnEntity(spawn2,
					EntityType.CHICKEN);
			SpecialChicken[] list = SpecialChicken.values();
			SpecialChicken sc = list[random.nextInt(list.length)];
			e2.setBaby();
			ChickenMetadataValue.setMetadataValueToChicken(main, e2,
					zone.getUniqueId(), sc);
			e2.setHealth(1);
		}
		e.setAdult();
		ChickenMetadataValue.setMetadataValueToChicken(main, e,
				zone.getUniqueId(), null);
		e.setHealth(1);
	}

	private Location randomLocationInZone(Location l1, Location l2) {
		double x = random.nextDouble() * Math.abs(l1.getX() - l2.getX())
				+ Math.min(l1.getX(), l2.getX());
		double y = random.nextDouble() * Math.abs(l1.getY() - l2.getY())
